...
def test_create_read_update_delete_transaction(app):
    ...
    # create transaction
    r = client.post('/api/transactions/', json={'account_id': aid, 'amount': 12.34, 'type': 'expense', 'description': 'coffee'}, headers=headers)
    ...
    # update
    u = client.put(f'/api/transactions/{txid}', json={'description': 'latte', 'amount': 15.0}, headers=headers)
